...
#
# Usage: ./bead-ralph.sh [--worktree] [--silent] <max-iterations> [epic-id...]
#
...
#   --silent      Suppress Claude's output (only show iteration markers and results)
#
# Prerequisites:
...
show_usage() {
    echo "Usage: $SCRIPT_NAME [--worktree] [--silent] <iterations> [epic-id...]"
    echo ""
    echo "Options:"
    echo "  --worktree    Run in isolated git worktree (auto-merges on completion)"
    echo "  --silent      Suppress Claude's streaming output (show only after completion)"
    echo ""
    echo "Examples:"
    ...
    echo "  $SCRIPT_NAME --silent 20 bd-a3f8     # Run without streaming output"
    exit 1
}
...
# Set up cleanup trap (handles both temp file and worktree if enabled)
if [ "$WORKTREE_MODE" = true ]; then
    trap 'rm -f "$TEMP_OUTPUT"; teardown_worktree "$WORKTREE_NAME" $?' EXIT
else
...
"
    ...
    if [ "$SILENT_MODE" = true ]; then
        # Silent mode: capture output without streaming
